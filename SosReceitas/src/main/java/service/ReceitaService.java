...
//Update em uma receita
    ...
    public Object addFavorito(Request request, Response response) {
    	  receitadao.conectar();

    	    Gson gson = new Gson();
    	    JsonObject json = gson.fromJson(request.body(), JsonObject.class);

    	    int idUser = json.get("userId").getAsInt();
    	    int idReceita = json.get("recipeId").getAsInt();

    	    boolean status = receitadao.addFavorito(idUser, idReceita);
    	    ...
    	    if (status) {
    	        ...
    	        return "Erro ao adicionar favorito";
    	    }
    }

    public Object getFavoritos(Request request, Response response) {
        receitadao.conectar();

        int idUser = Integer.parseInt(request.params(":id_user"));
        List<Receitas> favoritos = receitadao.getFavoritos(idUser);
        
        // Construir a URL base
        String baseUrl = request.scheme() + "://" + request.host();

        // Atualizar a URL da imagem para cada receita favorita
        for (Receitas receita : favoritos) {
            String imageUrl = baseUrl + "/assets/img/" + receita.getImagem();
            receita.setImagem(imageUrl);
        }
        ...
        return new Gson().toJson(favoritos);
    }
}
